...
# Use larger max length for data preprocessing (should be >= max scheduled length)
# IMPORTANT: Set this to the MAXIMUM length in your schedule so vLLM allocates enough KV cache
MAX_LEN=${1:-2048}
TEMPERATURE=${2:-1.0}
...
export N_GPUS=$(nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-"legal-rules_l2048-1024-512_t${TEMPERATURE}"}
...
trainer_args=" \
    ...
    trainer.response_length_schedule=[[1,2048],[33,1024],[67,512]] \
"
