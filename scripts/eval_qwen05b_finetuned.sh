...
export MODEL_DIR="outputs/test/test/0320-16:46_countdown-instruct_Qwen2.5-0.5B-Instruct_test/checkpoint/global_step_1"
export MODEL_PATH="${MODEL_DIR}/actor/huggingface"
export TOKENIZERS_PARALLELISM=false

# merge trained model
# comment out `delete_after_merge` flag if you want to keep the shareded weight
if [ -d "$MODEL_DIR/actor" ]; then
    echo "Merging model at: $MODEL_DIR/actor"
    python -m verl.utils.model_merger --local_dir "$MODEL_DIR/actor" --delete_after_merge
    echo "Finished merging: $MODEL_DIR/actor"
fi

TASKS=(
    #"mmlu" "gpqa_diamond" 
    "aime24" "gsm8k" "math500"
...
while (( task_idx < total_tasks )); do
    ...
    if (( available_gpu >= 0 )); then
        task="${TASKS[$task_idx]}"
        run_task "$MODEL_PATH" "$task" "$available_gpu"
        (( task_idx++ ))
    else
