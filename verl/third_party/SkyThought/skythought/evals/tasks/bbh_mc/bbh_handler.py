...
class BBHTaskHandler(TaskHandler):
    ...
    def generate_prompt(self, problem: Dict[str, Any]) -> str:
        template = self.task_config.templating_parameters["template"]
        current_prompt = template.format(**problem)

        fewshot_block = self.task_config.fewshot_prompt.strip().split("\n\n")
        num_fewshot = self.task_config.num_fewshot

        selected_fewshots = "\n\n".join(fewshot_block[:num_fewshot]) if fewshot_block else ""

        # Add extra spacing to separate from main prompt
        full_prompt = f"{selected_fewshots}\n\n{current_prompt}" if selected_fewshots else current_prompt
