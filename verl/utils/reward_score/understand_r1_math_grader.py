...
def compute_score(
    solution_str, ground_truth, method="qwen_math_template", format_reward=0.0, answer_reward=1.0
):
    if method in ["qwen_math_template", "no_template"]:
        math_reward_fn = boxed_reward_fn
    elif method in ["r1_template"]:
        math_reward_fn = answer_tag_reward_fn
    else:
        raise NotImplementedError("Specified template type reward parsing function has not been yet defined")
    
    format_correct, answer_correct = False, False
    info, answer_correct = math_reward_fn(solution_str, ground_truth)
    format_correct = info["formatted"]

    return format_correct * format_reward + answer_correct * answer_reward, answer_correct, {'format': format_correct, 'answer': answer_correct}
