...
def hf_generate_responses_batch(
    model,
    tokenizer,
    conversations,
    max_tokens,
    ...
    for i in tqdm(range(0, len(conversations), batch_size)):
        ...
        for idx, conversation in enumerate(batch):
            ...
            if num_return_sequences > 1:
                ...
                item_outputs = [
                    outputs[idx, seq_idx] for seq_idx in range(num_return_sequences)
                ]
            else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Batch size for Hugging Face inference.",
    )
