...
"""
The main entry point to run the GRPO algorithm
"""
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # normalize config
        if self._is_actor:
            self.config.actor.mini_batch_size *= self.config.rollout.n
            self.config.actor.mini_batch_size //= (
                self.device_mesh.shape[0] // self.ulysses_sequence_parallel_size
            )
            self.config.actor.micro_batch_size_per_gpu //= (
    ...
    def _build_model_optimizer(
        ...
        # torch_dtype = fsdp_config.get('model_dtype', torch.bfloat16)
        # print("Using bfloat16 as default!!!!!")
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == "hf":
            ...
            from verl.trainer.grpo.workers.vllm_rollout import vLLMRollout
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                if self.config.actor.use_token_level_loss:
                    metrics = self.actor.update_token_level_policy(data=data)
                else:
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # clear kv cache
        torch.cuda.empty_cache()
        log_gpu_memory_usage("After compute old_log_prob", logger=logger)
        return output
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        torch.cuda.empty_cache()
        log_gpu_memory_usage("After compute ref_log_prob", logger=logger)
        return output
